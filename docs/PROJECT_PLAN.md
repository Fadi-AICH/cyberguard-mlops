...
1. **Data Sources**: reproducible synthetic CIC-style network flows, with a documented path to replace them by CICIDS2017/KDD data.
2. **Data Ingestion**: CLI generator writes raw CSV with attack labels and realistic feature distributions.
...
10. **Monitoring**: Prometheus scrapes API metrics; Grafana dashboard visualizes latency, throughput, and attack-rate signals.
...
- Great Expectations is planned for formal data validation checkpoints; the project also includes a lightweight validator so the pipeline remains runnable if GX is not installed yet.
